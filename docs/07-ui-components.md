...
| planReports | PlanReport[] | 계획 목록 (승인/거절 버튼 표시용) |
| approvedPlans | Set\<number\> | 승인된 계획 인덱스 집합 |
| onApprovePlan | (index) => void | 계획 승인 콜백 |
| rejectedPlans | Set\<number\> | 거절된 계획 인덱스 집합 |
| onRejectPlan | (index) => void | 계획 거절 콜백 |
| meetingPhase | number | 현재 회의 Phase |
| onOpenMap | () => void | "지도" 클릭 시 FactionMap 열기 |

**UI 요소:**
- 참모 아이콘 + 색상 코드로 발언자 구분
- 이름 옆 충성도/열정: `StatIcon` 컴포넌트 (클릭 시 툴팁 표시) — 전략 역할(제갈량) 제외
- Phase 구분선 (`__phase_divider__` speaker)
- 클릭 시 하이라이트 (답장 대상 선택)
- 타이핑 인디케이터: 3개 점 바운스 애니메이션
- 쓰레드: 좌측 보더라인 + 들여쓰기
  - 쓰레드 메시지에도 충성도/열정 StatIcon 표시
  - `stat_delta` 있으면 말풍선 하단에 `🔥 열정 +N` / `♥ 충성도 +N` 표시
- "유비" 발언: 우측 정렬, 금색 말풍선
- 승인/거절 버튼: Phase 2 토론 중 각 참모의 마지막 메시지 하단에 표시
  - 포인트 예상 변동 요약 (컬러) + `extra_note` (회색) 표시
  - 승인: `✓ 승인됨` (초록), 거절: `✗ 거절됨` (빨간)
  - 실제 포인트/시설 변동은 실행(Phase 3) 시작 시 적용 (지연 실행)
