  ...
  return [
    {
      ...
      cost: `특수능력 ${SPECIAL_STRATEGY_SP_COST}`,
      costValue: SPECIAL_STRATEGY_SP_COST,
      successRate: currentRate,
      available: player.points.sp >= SPECIAL_STRATEGY_SP_COST,
      unavailableReason: player.points.sp < SPECIAL_STRATEGY_SP_COST ? "특수능력 부족" : undefined,
    },
    {
      type: "전투",
      label: "전투",
    ...
    },
    {
      ...
      cost: `외교력 ${SUPPORT_REQUEST_DP_COST}`,
      ...
      unavailableReason: player.points.dp < SUPPORT_REQUEST_DP_COST ? "외교력 부족" : otherFactions.length === 0 ? "요청할 세력 없음" : undefined,
    },
    {
      type: "조공",
      label: "조공",
      description: `재물을 바쳐 퇴각시킨다. (내정력 ${tributeCost})`,
      cost: `내정력 ${tributeCost}`,
      costValue: tributeCost,
      successRate: 1.0,
      available: player.points.ip >= tributeCost,
      unavailableReason: player.points.ip < tributeCost ? "내정력 부족" : undefined,
    },
  ];
}
