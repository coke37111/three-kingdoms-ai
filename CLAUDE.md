...
이 프로젝트의 **진짜 목적은 게임이 아니다.**

AI NPC와의 자연스러운 대화와 협업이 어떻게 가능한지, 그 원칙을 탐구하는 실험적 프로젝트다. 삼국지 전략 게임은 그 원칙을 검증하는 **프레임워크(테스트베드)** 로 사용된다.

**탐구 질문:**
- AI NPC가 단순 명령 실행자가 아닌, 맥락을 이해하고 능동적으로 협력하는 존재가 될 수 있는가?
- 사람과 AI 사이의 자연스러운 협업을 가능케 하는 대화 구조는 무엇인가?
- 역할, 관계, 상황 인식을 통해 AI의 발화가 얼마나 일관되고 신뢰할 수 있는가?

**작업 방식:**
- 게임 플레이를 통해 AI 대화의 어색함/비일관성/단절을 발견한다.
- 룰셋(프롬프트, 케이스, 템플릿)을 지속적으로 다듬어 원칙을 발굴한다.
- 발견한 원칙은 문서화하여 누적한다.

따라서 기능 구현보다 **대화 품질과 원칙 정립**이 항상 우선이다.
...
- `npm run build` 또는 `npm run dev` 실행 후 **서버 주소(localhost:포트)를 반드시 사용자에게 알려준다.**
